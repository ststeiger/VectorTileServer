...
*/
        ...
        )
        {
            ...
            UnpackedRecord
                aSpace = new UnpackedRecord(); //char aSpace[150]; /* Temp space for pIdxKey - to avoid a malloc */
...
*/
        static int defragmentPage(MemPage pPage)
        {
            ...
            Buffer.BlockCopy(data, cbrk, temp, cbrk,
                usableSize - cbrk); //memcpy( temp[cbrk], ref data[cbrk], usableSize - cbrk );
            cbrk = usableSize;
...
*/
        static int lockBtree(BtShared pBt)
        {
            ...
            */
            ...
            if (nPage == 0 || memcmp(pPage1.aData, 24, pPage1.aData, 92, 4) != 0
        ...
        static int sqlite3BtreeIncrVacuum(Btree p)
        {
            ...
            else
            {
                ...
                if (rc == SQLITE_OK)
                {
                    ...
                    sqlite3Put4byte(pBt.pPage1.aData, (u32) 28,
                        pBt.nPage); //put4byte(&pBt->pPage1->aData[28], pBt->nPage);
                }
            }
...
*/
        ...
        static void assertCellInfo(BtCursor pCur)
        {
            ...
            Debug.Assert(info.GetHashCode() == pCur.info.GetHashCode() ||
                         info.Equals(pCur.info)); //memcmp(info, pCur.info, sizeof(info))==0 );
        }
#else
...
*/
        ...
        )
        {
            if (eOp != 0)
            {
                ...
                Buffer.BlockCopy(pBuf, (int) pBufOffset, pPayload, (int) payloadOffset,
                    (int) nByte); // memcpy( pPayload, pBuf, nByte );
            }
            else
            {
                /* Copy data from page to buffer (a read operation) */
                Buffer.BlockCopy(pPayload, (int) payloadOffset, pBuf, (int) pBufOffset,
                    (int) nByte); //memcpy(pBuf, pPayload, nByte);
            }
...
*/
        ...
        )
        {
            ...
            if (offset < pCur.info.nLocal)
            {
                ...
                rc = copyPayload(aPayload, (u32) (offset + pCur.info.iCell + pCur.info.nHeader), pBuf, pBufOffset,
                    (u32) a, eOp, pPage.pDbPage);
                offset = 0;
                    ...
                    else
                    {
                        ...
                        */
                        ...
                        if (0 == (nCell & 0x80) && nCell <= pPage.maxLocal)
                        {
                            ...
                            ** b-tree page.  */
                            c = sqlite3VdbeRecordCompare(nCell, pPage.aData, pCell + 1,
                                pIdxKey); //c = sqlite3VdbeRecordCompare( nCell, (void*)&pCell[1], pIdxKey );
                        }
                        else if (0 == (pPage.aData[pCell + 1] & 0x80) //!(pCell[1] & 0x80)
                                 && (nCell = ((nCell & 0x7f) << 7) + pPage.aData[pCell + 1]) <=
                                 pPage.maxLocal //pCell[1])<=pPage.maxLocal
                        )
                        {
                            /* The record-size field is a 2 byte varint and the record
                            ** fits entirely on the main b-tree page.  */
                            c = sqlite3VdbeRecordCompare(nCell, pPage.aData, pCell + 2,
                                pIdxKey); //c = sqlite3VdbeRecordCompare( nCell, (void*)&pCell[2], pIdxKey );
                        }
                        else
                        {
                            ...
                            ** buffer before VdbeRecordCompare() can be called. */
                            ...
                            Buffer.BlockCopy(pPage.aData, pCell - pPage.childPtrSize, pCellBody, 0,
                                pCellBody.Length); //          u8 * const pCellBody = pCell - pPage->childPtrSize;
                ...
                do
                {
                    ...
                    if (k == 0 && 0 == searchList)
                    {
                        ...
                        ** allocated page */
                        ...
                        pPgno = iTrunk;
                        Buffer.BlockCopy(pTrunk.aData, 0, pPage1.aData, 32,
                            4); //memcpy( pPage1.aData[32], ref pTrunk.aData[0], 4 );
                        ppPage = pTrunk;
                        pTrunk = null;
                        ...
                        */
                        ...
                        else
                        {
                            ...
                            */
                            ...
                            Buffer.BlockCopy(pTrunk.aData, 12, pNewTrunk.aData, 8,
                                (int) (k - 1) * 4); //memcpy( pNewTrunk.aData[8], ref pTrunk.aData[12], ( k - 1 ) * 4 );
                            releasePage(pNewTrunk);
                            if (null == pPrevTrunk)
                            {
                    ...
                    else if (k > 0)
                    {
                        ...
                        if (0 == searchList || iPage == nearby)
                        {
                            ...
                            if (closest < k - 1)
                            {
                                Buffer.BlockCopy(aData, (int) (4 + k * 4), aData, 8 + (int) closest * 4,
                                    4); //memcpy( aData[8 + closest * 4], ref aData[4 + k * 4], 4 );
                            }
...
*/
        ...
        )
        {
            ...
            ** buffer space that is separate from the pPage buffer area */
            ...
            if (pPage.hasData != 0)
            {
                nHeader += (int) putVarint(pCell, nHeader,
                    (int) (nData + nZero)); //putVarint( pCell[nHeader], nData + nZero );
            }
            else
            {
                nData = nZero = 0;
        ...
        )
        {
            ...
            ** the term after the || in the following assert(). */
            ...
            if (pPage.nOverflow != 0 || sz + 2 > pPage.nFree)
            {
                if (pTemp != null)
                {
                    Buffer.BlockCopy(pCell, nSkip, pTemp, nSkip,
                        sz - nSkip); //memcpy(pTemp+nSkip, pCell+nSkip, sz-nSkip);
                    pCell = pTemp;
                }
            ...
            else
            {
                ...
                ** if it returns success */
                ...
                Buffer.BlockCopy(pCell, nSkip, data, idx + nSkip,
                    sz - nSkip); //memcpy( data[idx + nSkip], pCell + nSkip, sz - nSkip );
                if (iChild != 0)
                {
...
*/
        ...
        )
        {
            ...
            for (i = nCell - 1; i >= 0; i--)
            {
                ...
                put2byte(data, pCellptr, cellbody);
                Buffer.BlockCopy(apCell[offset + i], 0, data, cellbody,
                    aSize[i + offset]); //          memcpy(&data[cellbody], apCell[i], aSize[i]);
            }
...
*/
        static void copyNodeContent(MemPage pFrom, MemPage pTo, ref int pRC)
        {
            if ((pRC) == SQLITE_OK)
            {
                ...
                Buffer.BlockCopy(aFrom, iData, aTo, iData,
                    (int) pBt.usableSize - iData); //memcpy(aTo[iData], ref aFrom[iData], pBt.usableSize-iData);
                Buffer.BlockCopy(aFrom, iFromHdr, aTo, iToHdr,
                    pFrom.cellOffset +
                    2 * pFrom.nCell); //memcpy(aTo[iToHdr], ref aFrom[iFromHdr], pFrom.cellOffset + 2*pFrom.nCell);
            ...
            for (i = 0; i < nOld; i++)
            {
                ...
                ** process of being overwritten.  */
                ...
                if (i < nOld - 1 && 0 == leafData)
                {
                    ...
                    Buffer.BlockCopy(pTemp, leafCorrection, apCell[nCell], 0,
                        sz); //apCell[nCell] = pTemp + leafCorrection;
                    ...
                    if (0 == pOld.leaf)
                    {
                        ...
                        ** pointer of the divider cell */
                        Buffer.BlockCopy(pOld.aData, 8, apCell[nCell], 0,
                            4); //memcpy( apCell[nCell], ref pOld.aData[8], 4 );
                    }
                    else
                    {
...
if (false)
            ...
            */
            ...
            {
                Buffer.BlockCopy(apCopy[nOld - 1].aData, 8, apNew[nNew - 1].aData, 8,
                    4); //u8* zChild = &apCopy[nOld - 1].aData[8];
...
if (false)
            ...
            Array.Copy(pRoot.aOvfl, pChild.aOvfl,
                pRoot.nOverflow); //memcpy(pChild.aOvfl, pRoot.aOvfl, pRoot.nOverflow*sizeof(pRoot.aOvfl[0]));
        ...
        static int sqlite3BtreeDelete(BtCursor pCur)
        {
            ...
            ** node to replace the deleted cell.  */
            if (0 == pPage.leaf)
            {
                ...
                insertCell(pPage, iCellIdx, pNext_4, nCell + 4, null, n,
                    ref rc); //insertCell( pPage, iCellIdx, pCell - 4, nCell + 4, pTmp, n, ref rc );
        ...
        )
        {
            ...
            */
            ...
            //}else
            {
                ...
                Array.Clear(hit, contentOffset,
                    usableSize - contentOffset); //memset(hit+contentOffset, 0, usableSize-contentOffset);
