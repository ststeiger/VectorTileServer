    ...
    sqlite3AuthCheck(pParse, SQLITE_CREATE_VTABLE, pTable->zName, 
            ...
            */
            if (0 == db.init.busy)
            {
                ...
                zStmt = sqlite3MPrintf(db, "CREATE VIRTUAL TABLE %T",
                    pParse.sNameToken.z.Substring(0, pParse.sNameToken.n));
                ...
                else
                {
                    ...
                    ** the type string.  */
                    ...
                    for (iCol = 0; iCol < pTab.nCol; iCol++)
                    {
                        ...
                        if (sqlite3StrNICmp("hidden", 0, zType.ToString(), 6) != 0 ||
                            (zType.Length > 6 && zType[6] != ' '))
                        {
                            for (i = 0; i < nType; i++)
                            {
        ...
        */
        static void sqlite3VtabMakeWritable(Parse pParse, Table pTab)
        {
            ...
            n = pToplevel.apVtabLock == null
                ? 1
                : pToplevel.apVtabLock.Length + 1; //(pToplevel.nVtabLock+1)*sizeof(pToplevel.apVtabLock[0]);
