            ...
            */
            ...
            else
            {
                ...
                if (Double.IsNegativeInfinity(pMem.r))
                    ...
                    pMem.z = pMem.r.ToString(CultureInfo.InvariantCulture)
                        .ToLower(); //sqlite3_snprintf(nByte, pMem.z, "%!.15g", pMem->r);
                else
        ...
        */
        static int sqlite3VdbeMemNumerify(Mem pMem)
        {
            if ((pMem.flags & (MEM_Int | MEM_Real | MEM_Null)) == 0)
            {
                ...
                {
                    if (0 == sqlite3Atoi64(Encoding.UTF8.GetString(pMem.zBLOB, 0, pMem.zBLOB.Length), ref pMem.u.i,
                            pMem.n, pMem.enc))
                        MemSetTypeFlag(pMem, MEM_Int);
                    else
                    {
        ...
        */
        ...
        )
        {
            ...
            if (nByte < 0)
            {
                ...
                {
                    for (nByte = 0;
                        nByte <= iLimit && nByte < zBlob.Length - offset && zBlob[offset + nByte] != 0;
                        nByte++)
                    {
                    }
                }
                ...
                {
                    for (nByte = 0;
                        nByte <= iLimit && zBlob[nByte + offset] != 0 || zBlob[offset + nByte + 1] != 0;
                        nByte += 2)
                    {
                    }
                }
        ...
        )
        {
            ...
            */
            if (xDel == SQLITE_TRANSIENT)
            {
                ...
                {
                    pMem.z = n > 0 && z.Length - offset > n
                        ? z.Substring(offset, n)
                        : z.Substring(offset); //memcpy(pMem.z, z, nAlloc);
                    sqlite3_free(ref pMem.zBLOB);
                }
            }
            else if (xDel == SQLITE_DYNAMIC)
            {
                ...
                if (enc == 0)
                {
                    ...
                    if (pMem.zBLOB != null)
                        sqlite3_free(ref pMem.zBLOB);
                    pMem.zBLOB = Encoding.UTF8.GetBytes(offset == 0 ? z :
                        z.Length + offset < n ? z.Substring(offset, n) : z.Substring(offset));
                }
                else
                {
                    pMem.z = n > 0 && z.Length - offset > n
