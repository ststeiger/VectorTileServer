    ...
    */
        ...
        static void sqlite3RootPageMoved(sqlite3 db, int iDb, int iFrom, int iTo)
        {
            ...
            for (pElem = pHash.first;
                pElem != null;
                pElem = pElem.next) // ( pElem = sqliteHashFirst( pHash ) ; pElem ; pElem = sqliteHashNext( pElem ) )
            {
...
return;
        ...
        */
        // OVERLOADS, so I don't need to rewrite parse.c
        static Index sqlite3CreateIndex(Parse pParse, int null_2, int null_3, int null_4, int null_5, int onError,
            int null_7, int null_8, int sortOrder, int ifNotExist)
        {
            return sqlite3CreateIndex(pParse, null, null, null, null, onError, null, null, sortOrder, ifNotExist);
        }

        static Index sqlite3CreateIndex(Parse pParse, int null_2, int null_3, int null_4, ExprList pList, int onError,
            int null_7, int null_8, int sortOrder, int ifNotExist)
        {
...
goto exit_drop_index;
        ...
        */
        // OVERLOADS, so I don't need to rewrite parse.c
        static SrcList sqlite3SrcListAppendFromTerm(Parse pParse, SrcList p, int null_3, int null_4, Token pAlias,
        ...
        static SrcList sqlite3SrcListAppendFromTerm(Parse pParse, SrcList p, Token pTable, Token pDatabase,
        ...
        static void reindexDatabases(Parse pParse, string zColl)
        {
            ...
            for (iDb = 0; iDb < db.nDb; iDb++) //, pDb++ )
            {
                ...
                for (k = pDb.pSchema.tblHash.first;
                    k != null;
                    k = k.next) //for ( k = sqliteHashFirst( pDb.pSchema.tblHash ) ; k != null ; k = sqliteHashNext( k ) )
                {
