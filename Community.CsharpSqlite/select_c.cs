...
namespace Community.CsharpSqlite
{
    public partial class Sqlite3
    {
        ...
        */
        ...
        static Select sqlite3SelectNew(Parse pParse, int null_2, SrcList pSrc, int null_4, int null_5, int null_6,
            int null_7, int isDistinct, int null_9, int null_10)
        {
            ...
            for (i = 0; i < 3 && apAll[i] != null; i++)
            {
                ...
                for (j = 0; j < ArraySize(aKeyword); j++)
                {
                    if (p.n == aKeyword[j].nChar
                        && p.z.StartsWith(zKeyText.Substring(aKeyword[j].i, aKeyword[j].nChar),
                ...
                else if ((p.op == TK_COLUMN || p.op == TK_AGG_COLUMN) && pTabList != null)
                {
                    ...
                    if (!shortNames && !fullNames)
                    {
                        sqlite3VdbeSetColName(v, i, COLNAME_NAME,
                            pEList.a[i].zSpan,
                            SQLITE_DYNAMIC); //sqlite3DbStrDup(db, pEList->a[i].zSpan), SQLITE_DYNAMIC);
                    }
                    else if (fullNames)
                    {
                        string zName;
        ...
        */
        ...
        )
        {
            ...
            for (i = 0; i < nCol; i++) //, pCol++ )
            {
                ...
                string bDummy = null;
                pCol.zType =
                    columnType(sNC, p, ref bDummy, ref bDummy,
                        ref bDummy); // sqlite3DbStrDup( db, columnType( sNC, p, 0, 0, 0 ) );
        ...
        )
        {
            ...
            */
            ...
            if (aPermute != null)
            {
                ...
                pKeyMerge =
                    new KeyInfo(); //      sqlite3DbMallocRaw(db, sizeof(*pKeyMerge)+nOrderBy*(sizeof(CollSeq)+1));
                if (pKeyMerge != null)
                {
