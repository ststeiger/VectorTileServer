...
namespace Community.CsharpSqlite
{
    public partial class Sqlite3
    {
        ...
        ******************************************************************************
        **
        ** This file contains code that is specific to windows. This is
        ** meaningless for the supposedly platform-neutral .Net.  This now contains
        ** experimental changes to allow Linux/Mac OS X use.  The ultimate solution
        ** should be substantially cleaner than this version, however.
    ...
    */
        static int seekWinFile(sqlite3_file id, sqlite3_int64 iOffset)
        {
            ...
            */
            ...
            {
                id.fs.Seek(iOffset,
                    SeekOrigin.Begin); // SetFilePointer(pFile.fs.Name, lowerBits, upperBits, FILE_BEGIN);
            }
            catch (Exception e)
            {
#if SQLITE_SILVERLIGHT
...
*/
    ...
    static tcl.lang.Var.SQLITE3_GETSET sqlite3_fullsync_count =
 new tcl.lang.Var.SQLITE3_GETSET( "sqlite_fullsync_count" );
#endif
#endif
      ...
      */
      ...
      if ( SimulateIOError() )
        return SQLITE_IOERR_FSTAT;
            ...
            ////return SQLITE_OK;
            FileInfo fi = new FileInfo(id.fs.Name);
            pSize = fi.Length;
            return SQLITE_OK;
        }
        ...
        */
        static int unlockReadLock(sqlite3_file pFile)
        {
            ...
            if (isNT())
            {
                try
                {
                    lockingStrategy.UnlockFile(pFile, SHARED_FIRST,
                        SHARED_SIZE); //     res = UnlockFile(pFile.h, SHARED_FIRST, 0, SHARED_SIZE, 0);
                }
                catch (Exception e)
                {
                    res = 0;
      ...
      */
            ...
            */
            ...
            )
            {
                ...
                res = 0;
                while (cnt-- > 0 && res == 0
                ) //(res = LockFile(pFile.fs.SafeFileHandle.DangerousGetHandle().ToInt32(), PENDING_BYTE, 0, 1, 0)) == 0)
                {
                    try
                    {
...
error = 1;
            ...
            */
            if ((locktype == RESERVED_LOCK) && res != 0)
            {
                ...
                try
                {
                    lockingStrategy.LockFile(pFile, RESERVED_BYTE,
                        1); //res = LockFile(pFile.fs.SafeFileHandle.DangerousGetHandle().ToInt32(), RESERVED_BYTE, 0, 1, 0);
...
error = 1;
        ...
        */
        static int winUnlock(sqlite3_file id, int locktype)
        {
            ...
            {
                lockingStrategy.UnlockFile(pFile, SHARED_FIRST,
                    SHARED_SIZE); // UnlockFile(pFile.h, SHARED_FIRST, 0, SHARED_SIZE, 0);
            ...
            if (type >= PENDING_LOCK)
            {
                try
                {
                    lockingStrategy.UnlockFile(pFile, PENDING_BYTE,
                        1); //    UnlockFile(pFile.h, PENDING_BYTE, 0, 1, 0);
                }
                catch (Exception e)
                {
                }
...
#if WINDOWS_PHONE || SQLITE_SILVERLIGHT
 fs =
 new IsolatedStorageFileStream(zConverted, dwCreationDisposition, dwDesiredAccess, dwShareMode, IsolatedStorageFile.GetUserStoreForApplication());
...
#if WINDOWS_PHONE
          pResOut =
 System.IO.IsolatedStorage.IsolatedStorageFile.GetUserStoreForApplication().FileExists(zFilename) ? 1 : 0;
#elif SQLITE_SILVERLIGHT
...
#if SQLITE_WINRT
            rc =
 attr != FileAttributes.Normal ? 0 : (int)( attr & FileAttributes.ReadOnly ) != 0 ? 0 : 1; //FILE_ATTRIBUTE_READONLY ) == 0;
#else
                    rc = attr == 0 ? 0 :
                        (int) (attr & FileAttributes.ReadOnly) != 0 ? 0 : 1; //FILE_ATTRIBUTE_READONLY ) == 0;
#endif
                    break;
                default:
...
////#endif
    ...
    internal static class HelperMethods
    {
        public static bool IsRunningMediumTrust()
        {
            ...
            // For the purpose of running this on Mac OS X, we *always* run in medium trust mode.
////#if SQLITE_SILVERLIGHT
            return true;
////#else
////      return false;
