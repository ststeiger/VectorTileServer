...
namespace Community.CsharpSqlite
{
    ...
    public partial class Sqlite3
    {
        ...
        */
        static int getDigits(string zDate, int N0, int min0, int max0, char nextC0, ref int pVal0, int N1, int min1,
            int max1, char nextC1, ref int pVal1)
        {
            int c0 = getDigits(zDate + '\0', N0, min0, max0, nextC0, ref pVal0);
            return c0 == 0
                ? 0
                : c0 + getDigits(zDate.Substring(zDate.IndexOf(nextC0) + 1) + '\0', N1, min1, max1, nextC1, ref pVal1);
        }

        static int getDigits(string zDate, int N0, int min0, int max0, char nextC0, ref int pVal0, int N1, int min1,
            int max1, char nextC1, ref int pVal1, int N2, int min2, int max2, char nextC2, ref int pVal2)
        {
            int c0 = getDigits(zDate + '\0', N0, min0, max0, nextC0, ref pVal0);
            if (c0 == 0)
                return 0;
            ...
            if (c1 == 0)
                return c0;
            return c0 + c1 + getDigits(zDate1.Substring(zDate1.IndexOf(nextC1) + 1) + '\0', N2, min2, max2, nextC2,
                       ref pVal2);
        }
...
//} while ( nextC != 0 && zIndex < zDate.Length );
        ...
        */
        static int parseYyyyMmDd(string zDate, DateTime p)
        {
            ...
            if (getDigits(zDate.Substring(zIndex), 4, 0, 9999, '-', ref Y, 2, 1, 12, '-', ref M, 2, 1, 31, '\0',
                    ref D) != 3)
            {
                return 1;
            }
...
#if !SQLITE_OMIT_LOCALTIME
            {
                ...
                case 'u':
                {
                    ...
                    */
                    ...
                    {
                        p.iJD = (long) ((p.iJD + 43200) / 86400 +
                                        210866760000000L); //p->iJD = p->iJD/86400 + 21086676*(i64)10000000;
                        clearYMD_HMS_TZ(p);
                        rc = 0;
                    }
#if !SQLITE_OMIT_LOCALTIME
                ...
                case 'w':
                {
                    ...
                    */
                    if (z.ToString().StartsWith("weekday ")
                        && sqlite3AtoF(z.ToString().Substring(8), ref r, sqlite3Strlen30(z.ToString().Substring(8)),
                            SQLITE_UTF8)
                        && (n = (int) r) == r && n >= 0 && r < 7)
                    {
                        sqlite3_int64 Z;
